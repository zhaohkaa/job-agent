...
class DatabaseManager:
    ...
    def insert_application(self, job_id: str, status: str = "applied", note: str = ""):
        """记录一次投递"""
        conn = self._get_conn()
        cursor = conn.cursor()
        try:
            ...
            logger.debug("插入投递记录失败: %s", e)
        finally:
            conn.close()

    def update_job_status(self, job_id: str, status: str):
        """更新岗位状态（new/matched/applied/interview/rejected）"""
        conn = self._get_conn()
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE jobs SET status = ?, updated_at = datetime('now', 'localtime') WHERE job_id = ?",
            (status, job_id),
        )
        conn.commit()
        conn.close()
    ...
    def get_weekly_stats(self) -> dict:
        ...
        cursor.execute("""
            SELECT date(created_at) as day, COUNT(*) as total,
                   AVG(match_score) as avg_score, MAX(match_score) as max_score
            FROM jobs
            WHERE created_at >= datetime('now', '-7 days', 'localtime')
            GROUP BY day ORDER BY day
        """)
        daily_jobs = [dict(row) for row in cursor.fetchall()]

        # 热门公司 TOP10
        cursor.execute("""
            SELECT company_name, COUNT(*) as cnt, AVG(match_score) as avg_score
            FROM jobs
            WHERE created_at >= datetime('now', '-7 days', 'localtime')
              AND company_name != '' AND company_name != '未知公司'
        ...
        cursor.execute("""
            ...
            WHERE created_at >= datetime('now', '-7 days', 'localtime')
              AND salary != '' AND salary != '薪资面议'
        """)
        salary_data = [row["salary"] for row in cursor.fetchall()]
        ...
        cursor.execute("""
            ...
            WHERE created_at >= datetime('now', '-7 days', 'localtime')
        """)
        app_stats = dict(cursor.fetchone()) if cursor.rowcount > 0 else {}

        # 消息统计（面试邀请数）
        cursor.execute("""
            SELECT COUNT(*) as interview_invites
            FROM messages
            WHERE msg_type = 'interview_invite'
              AND created_at >= datetime('now', '-7 days', 'localtime')
        """)
        msg_stats = dict(cursor.fetchone()) if cursor.rowcount > 0 else {}
        ...
        return {
            ...
            "total_applied": app_stats.get("total_applied", 0),
            "interview_count": app_stats.get("interview_count", 0) + msg_stats.get("interview_invites", 0),
        }
    ...
    def get_today_application_count(self) -> int:
        """获取今日投递数（兼容别名）"""
        return self.get_today_apply_count()
