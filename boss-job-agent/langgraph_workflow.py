...
from config import SEARCH_CONFIG, AI_CONFIG, JOB51_CONFIG, INTERACTIVE_MODE, AUTO_APPLY
from browser_tool import Job51Browser
from ai_matcher import match_jd_to_resume, batch_match_jobs, pre_filter_salary, check_company_filter
...
async def morning_ai_match_node(state: MorningState) -> MorningState:
    ...
    try:
        # 优先使用预筛选结果，否则直接用 jobs_with_jd
        pre_filtered = state.get("pre_filtered_jobs", [])
        if pre_filtered:
            jobs = pre_filtered
            logger.info("[早间流程] 使用预筛选结果: %d 个岗位", len(jobs))
        else:
            jobs = state.get("jobs_with_jd", [])
            logger.info("[早间流程] 未启用预筛选，使用全量: %d 个岗位", len(jobs))
...
async def morning_pre_filter_node(state: MorningState) -> MorningState:
    """
    节点3.5: 预筛选（薪资+公司黑白名单）
    在 AI 打分前用规则过滤明显不合适的岗位，节省 LLM API 调用成本。
    """
    logger.info("[早间流程] 节点: 预筛选（薪资+公司）")
    ...
    try:
        ...
        for job in jobs:
            ...
            if not pre_filter_salary(job):
                ...
                continue
            # 公司黑白名单
            should_skip, _ = check_company_filter(job)
            if should_skip:
                skipped += 1
                continue
            kept.append(job)

        state["pre_filtered_jobs"] = kept
        state["pre_filter_skipped"] = skipped
        logger.info("[早间流程] 预筛选完成: %d 保留, %d 过滤",
                     len(kept), skipped)

    except Exception as e:
        state["error"] = f"预筛选异常: {str(e)}"
        state["error_count"] += 1
        logger.error("[早间流程] 预筛选异常: %s", e)

    return state


async def morning_interactive_review_node(state: MorningState) -> MorningState:
    """
    节点7: 交互式审阅模式
    在终端展示高匹配岗位，让用户手动勾选感兴趣的目标，再决定是否推送/投递。
    如果未启用交互模式，直接跳过。
    """
    logger.info("[早间流程] 节点: 交互式审阅")
    ...
    if not new_jobs:
        logger.info("[早间流程] 无高匹配岗位，跳过审阅")
        state["reviewed_jobs"] = []
        return state

    if not INTERACTIVE_MODE.get("enabled", False):
        logger.info("[早间流程] 交互模式未启用，自动通过所有岗位")
        state["reviewed_jobs"] = new_jobs
        return state

    # 交互式审阅：在终端展示岗位列表，让用户勾选
    try:
        top_n = INTERACTIVE_MODE.get("candidate_count", 20)
        candidates = new_jobs[:top_n]

        print("\n" + "=" * 60)
        print("📋 高匹配岗位审阅")
        print("=" * 60)
        print(f"共 {len(candidates)} 个候选岗位（按分数降序排列）\n")

        for i, job in enumerate(candidates, 1):
            score = job.get("match_score", 0)
            keyword_bonus = job.get("keyword_bonus", 0)
            company_bonus = job.get("company_bonus", 0)
            print(f"  [{i}] ⭐{score:.1f}  {job.get('job_name', '?')[:40]}")
            print(f"       🏢 {job.get('company_name', '?')[:30]}")
            print(f"       💰 {job.get('salary', '?')}")
            print(f"       📍 {job.get('city', '?')}")
            bonus_info = ""
            if keyword_bonus:
                bonus_info += f" 关键词{keyword_bonus:+.1f}"
            if company_bonus:
                bonus_info += f" 公司{company_bonus:+.1f}"
            if bonus_info:
                print(f"       📊 AI原始:{job.get('ai_raw_score', score):.1f}{bonus_info}")
            reason = job.get('match_reason', '')[:80]
            if reason:
                print(f"       💡 {reason}")
            print()

        print("-" * 60)
        print("输入要保留的岗位编号（逗号分隔，如: 1,3,5），直接回车=全部保留，q=退出")
        choice = input("> ").strip()

        if choice.lower() == 'q':
            logger.info("[早间流程] 用户取消审阅")
            state["reviewed_jobs"] = []
            return state

        if not choice:
            # 全部保留
            state["reviewed_jobs"] = candidates
            logger.info("[早间流程] 用户保留全部 %d 个岗位", len(candidates))
        else:
            # 解析编号
            try:
                indices = [int(x.strip()) for x in choice.split(",") if x.strip().isdigit()]
                selected = [candidates[idx - 1] for idx in indices if 1 <= idx <= len(candidates)]
                state["reviewed_jobs"] = selected
                logger.info("[早间流程] 用户勾选 %d/%d 个岗位", len(selected), len(candidates))
            except (ValueError, IndexError):
                logger.warning("[早间流程] 输入格式错误，保留全部")
                state["reviewed_jobs"] = candidates

    except EOFError:
        # 非交互环境（如 cron 任务），自动通过
        logger.info("[早间流程] 非交互环境，自动通过")
        state["reviewed_jobs"] = new_jobs
    except Exception as e:
        logger.warning("[早间流程] 交互审阅异常: %s，自动通过", e)
        state["reviewed_jobs"] = new_jobs

    return state


async def morning_auto_apply_node(state: MorningState) -> MorningState:
    """
    节点7.5: 自动投递高匹配岗位（≥8.5分）
    需在 config.py 中启用 AUTO_APPLY["enabled"] = True
    """
    logger.info("[早间流程] 节点: 自动投递")

    if state.get("error"):
        return state

    if not AUTO_APPLY.get("enabled", False):
        logger.info("[早间流程] 自动投递未启用，跳过")
        state["apply_results"] = []
        state["apply_count"] = 0
        return state

    reviewed = state.get("reviewed_jobs", state.get("new_jobs", []))
    if not reviewed:
        logger.info("[早间流程] 无岗位可投递")
        state["apply_results"] = []
        state["apply_count"] = 0
        return state
    ...
    try:
        ...
        max_daily = AUTO_APPLY.get("max_daily", 5)
        remaining = max(0, max_daily - today_applied)
        logger.info("[早间流程] 今日已投递 %d, 剩余额度 %d", today_applied, remaining)

        if remaining <= 0:
            logger.info("[早间流程] 今日投递额度已用完")
            state["apply_results"] = []
            state["apply_count"] = 0
            return state

        browser = await get_browser()
        min_score = AUTO_APPLY.get("min_score", 8.5)
        confirm = AUTO_APPLY.get("confirm_before_apply", True)

        results = await browser.batch_apply(
            reviewed,
            min_score=min_score,
            max_apply=remaining,
            confirm=confirm,
        ...
        state["apply_count"] = sum(1 for r in results if r.get("success"))

        logger.info("[早间流程] 自动投递完成: %d 次尝试, %d 成功",
                     len(results), state["apply_count"])

    except Exception as e:
        logger.error("[早间流程] 自动投递异常: %s", e)
        state["apply_results"] = []
        state["apply_count"] = 0
...
def build_morning_graph() -> StateGraph:
    ...
    状态流转 (v3.1 — LangGraph 条件边):
    START → login → search → collect_details → pre_filter → ai_match
        → deduplicate → interactive_review → auto_apply → generate_report → push → END

    每个节点后都有条件边: error? → handle_error → (retry 当前节点 or END)
    """
    ...
    workflow.add_node("pre_filter", morning_pre_filter_node)
    workflow.add_node("ai_match", morning_ai_match_node)
    workflow.add_node("deduplicate", morning_deduplicate_node)
    workflow.add_node("interactive_review", morning_interactive_review_node)
    workflow.add_node("auto_apply", morning_auto_apply_node)
    ...
    # 带条件边的节点列表（出错自动路由到 handle_error，成功路由到下一节点）
    _add_conditional_edges_morning(workflow)

    return workflow


def _add_conditional_edges_morning(workflow: StateGraph):
    """为早间工作流的每个关键节点添加条件边（正常→下一节点，出错→handle_error）"""

    # 节点流转链: (当前节点, 下一节点)
    flow = [
        ("login", "search"),
        ("search", "collect_details"),
        ("collect_details", "pre_filter"),
        ("pre_filter", "ai_match"),
        ("ai_match", "deduplicate"),
        ("deduplicate", "interactive_review"),
        ("interactive_review", "auto_apply"),
        ("auto_apply", "generate_report"),
        ("generate_report", "push"),
    ]

    for current_node, next_node in flow:
    ...
    # handle_error 之后的条件路由: 如果错误恢复 → 继续到 push 前的逻辑
    # 简化: handle_error → generate_report（如果登录恢复则生成报告，否则直接结束）
    workflow.add_conditional_edges(
        "handle_error",
        _route_after_error_handler,
        {
            "continue": "push",     # 不重试复杂节点，直接推空报告
            "end": END,
        },
    )
...
def _route_after_node(state: MorningState) -> str:
    """判断节点执行后是继续下一步还是进入异常处理"""
    if state.get("error"):
        logger.warning("路由: 检测到错误 '%s' → handle_error", str(state["error"])[:60])
        return "handle_error"
    return "continue"


def _route_after_error_handler(state: MorningState) -> str:
    """异常处理后的路由"""
    if state.get("error_count", 0) > 3:
        logger.error("路由: 错误次数过多 → END")
        return "end"
    if state.get("is_logged_in") and not state.get("error"):
        logger.info("路由: 错误已恢复 → continue")
        return "continue"
    # 登录未恢复但有其他异常，也继续（推送空报告通知用户）
    return "continue"
...
def build_evening_graph() -> StateGraph:
    ...
    # 晚间流程链
    evening_flow = [
        ("login", "fetch_messages"),
        ("fetch_messages", "stats"),
        ("stats", "generate_report"),
        ("generate_report", "push"),
    ]

    for current_node, next_node in evening_flow:
        workflow.add_conditional_edges(
            current_node,
            _route_evening,
            {
...
def _route_evening(state: EveningState) -> str:
    """晚间节点路由"""
    if state.get("error"):
        return "handle_error"
    return "continue"
...
def _route_evening_after_error(state: EveningState) -> str:
    """晚间异常处理后路由"""
    if state.get("error_count", 0) > 3:
        return "end"
    if state.get("is_logged_in") and not state.get("error"):
...
async def run_morning_workflow(
    ...
    """
    执行早间搜岗工作流（LangGraph 驱动）。

    使用 LangGraph compiled.ainvoke() 自动管理状态流转、
    条件路由（出错→handle_error→重试）、checkpoint 持久化。
    """
    logger.info("=" * 60)
    logger.info("开始执行早间搜岗工作流 (LangGraph v3.1)")
    logger.info("=" * 60)

    task_log_id = db.log_task_start("morning")

    # 使用日期作为 thread_id，同一日历日重复执行可恢复
    thread_id = f"morning_{datetime.now().strftime('%Y%m%d')}"
    ...
    try:
        # 构建图 + 编译（使用 MemorySaver 做 checkpoint，支持中断恢复）
        graph = build_morning_graph()
        memory = MemorySaver()
        compiled = graph.compile(checkpointer=memory)

        # LangGraph 驱动执行：自动走状态图 + 条件边
        config = {"configurable": {"thread_id": thread_id}}
        final_state = await compiled.ainvoke(initial_state, config)

        # 保存最终状态到数据库
        db.save_workflow_state("morning", "last_state", {
            "matched_count": len(final_state.get("new_jobs", final_state.get("matched_jobs", []))),
            "raw_count": len(final_state.get("raw_jobs", [])),
            "pre_filter_skipped": final_state.get("pre_filter_skipped", 0),
            "apply_count": final_state.get("apply_count", 0),
        ...
        summary = {
            ...
            "matched_jobs": len(final_state.get("new_jobs", final_state.get("matched_jobs", []))),
            "raw_jobs": len(final_state.get("raw_jobs", [])),
            "pre_filter_skipped": final_state.get("pre_filter_skipped", 0),
            "apply_count": final_state.get("apply_count", 0),
            "report": final_state.get("report_content", ""),
            "push_result": final_state.get("push_result", {}),
            "error": final_state.get("error"),
        }

        db.log_task_end(task_log_id, summary["status"], summary)
        logger.info("早间工作流执行完成: %s (匹配:%d, 过滤:%d, 投递:%d)",
                     summary["status"], summary["matched_jobs"],
                     summary["pre_filter_skipped"], summary["apply_count"])
...
async def run_evening_workflow() -> dict:
    """
    执行晚间回复监控工作流（LangGraph 驱动）。
    返回执行结果摘要。
    """
    logger.info("=" * 60)
    logger.info("开始执行晚间回复监控工作流 (LangGraph v3.1)")
    logger.info("=" * 60)

    task_log_id = db.log_task_start("evening")
    thread_id = f"evening_{datetime.now().strftime('%Y%m%d')}"
    ...
    try:
        ...
        db.log_task_end(task_log_id, summary["status"], summary)
        logger.info("晚间工作流执行完成: %s (消息:%d)",
                     summary["status"], summary["message_count"])
