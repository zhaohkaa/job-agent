...
| 维度 | Boss 直聘 (v1.0) | 51job (v2.0) |
|------|------------------|-------------|
| 反爬难度 | 极高（JS加密+行为检测） | 较低（IP频率限制为主） |
| 登录方式 | App 扫码 | 短信/密码 |
| 搜索参数 | 前端加密，难构造 | URL 明文参数 |
| 晚间监控 | 消息中心（聊天消息） | 投递反馈 + 面试邀请页 |
| 稳定性 | 低（频繁更新反爬策略） | 中高（页面结构较稳定） |
...
### Q: 登录超时
**A:** 在 Chrome 中手动打开 51job.com 登录后，程序会自动检测到登录状态（每 5 秒检测一次，最长等 180 秒）。

### Q: 搜索不到岗位
**A:** 检查搜索关键词是否过于严格、城市编码是否正确。可在 `config.py` 中调整搜索条件。也可以先手动在 51job.com 上验证该关键词是否有结果。

### Q: 通义千问 API 调用失败
**A:** 检查 `DASHSCOPE_API_KEY` 环境变量是否正确设置。免费额度用完后需充值。

### Q: 微信收不到推送
**A:** 
- Server酱：检查 `SCT_SENDKEY` 是否正确设置
- 企业微信：检查 Webhook URL 是否有效

### Q: 如何切换回 Boss 直聘版本？
**A:** git 回退或查看 git log 找到 v1.0 的 commit。建议优先使用 51job 版本，稳定性更高。
