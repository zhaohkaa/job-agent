...
class AdaptiveRateLimiter:
    """
    自适应速率控制器。
    当检测到反爬验证页面时自动增加延迟，冷却后逐渐恢复。
    """
    def __init__(self, base_min: float = 2.0, base_max: float = 4.0):
        self.hit_count = 0         # 触发反爬的次数
        self.success_streak = 0     # 连续成功次数
        self.base_min = base_min
        self.base_max = base_max

    def on_verification_hit(self):
        """通知触发了一次反爬验证"""
        self.hit_count += 1
    ...
    def on_success(self):
        ...
        # 连续成功 10 次后，逐步恢复速度
        if self.success_streak >= 10 and self.hit_count > 0:
    ...
    def multiplier(self) -> float:
        """当前等待时间的倍率"""
        # 每次触发 +50% 等待时间，最多 5 倍
        return min(5.0, 1.0 + self.hit_count * 0.5)
...
_rate_limiter = AdaptiveRateLimiter(
    base_min=JOB51_CONFIG.get("min_wait", 2.0),
    base_max=JOB51_CONFIG.get("max_wait", 4.0),
)
...
class Job51Browser:
    ...
    def __init__(self):
        ...
        # 自适应限速：跟踪反爬触发次数，动态调整等待时间
        self._verification_hit_count = 0
        self._base_min_wait = JOB51_CONFIG.get("min_wait", 2.0)
        self._base_max_wait = JOB51_CONFIG.get("max_wait", 4.0)
    ...
    async def start(self):
        ...
        try:
            ...
            error_msg = str(e)
            # Chrome 版本与 Playwright 协议不兼容时的特殊处理
            if "Browser.setDownloadBehavior" in error_msg:
                logger.warning("检测到 CDP 协议兼容性问题（Chrome 版本过新/过旧）")
                logger.warning("尝试使用 channel 方式连接...")
                try:
                    self.browser = await self.playwright.chromium.connect_over_cdp(
                        self.cdp_url,
                        # 较新版本的 Chrome 需要设置此 header
                        headers={"x-playwright-launch-options": '{"headless":false}'},
                    )
                except Exception:
                    pass
            ...
            if not self.browser:
                ...
                logger.error(
                    "如果连接仍失败，请运行: pip install --upgrade playwright && playwright install chromium"
                )
                raise RuntimeError(
                    f"Chrome CDP 连接失败。请先执行:\n"
                    f"  open -a 'Google Chrome' --args --remote-debugging-port=9222\n"
                    f"然后在 Chrome 中登录 51job.com 后再运行本程序。\n"
                    f"如仍失败，运行: pip install --upgrade playwright && playwright install chromium\n"
                    f"原始错误: {e}"
                )
        ...
        try:
            ...
            for sel in name_link_selectors:
                try:
                    ...
                    if el:
                        raw_name = (await el.inner_text()).strip()
                        # 只取第一行作为岗位名，去除公司描述等冗余信息
                        job_name = raw_name.split("\n")[0].strip()
    ...
    async def fetch_jd_detail(self, job: dict) -> dict:
        ...
        try:
            ...
            if any(kw in page_text for kw in ["验证", "验证码", "滑块", "频繁", "限制"]):
                _rate_limiter.on_verification_hit()
                wait_sec = _rate_limiter.multiplier * 10
                logger.warning("检测到反爬验证页面 (第%d次)，等待 %.0f 秒后重试...",
                               _rate_limiter.hit_count, wait_sec)
            ...
            logger.info(
                "JD 获取完成: %d 字符 (倍率:%.1fx, url: %s...)",
                len(job.get("jd_text", "")),
                _rate_limiter.multiplier,
                job_link[:60],
            )
    ...
    async def apply_to_job(self, job: dict, confirm: bool = True) -> dict:
        """
        自动投递单个岗位。
        返回: {"success": bool, "message": str}
        """
        result = {"success": False, "message": "", "job_name": job.get("job_name", "")}

        job_link = job.get("job_link", "")
        if not job_link:
            result["message"] = "缺少岗位链接"
            return result

        try:
            job_name = job.get("job_name", "?")[:40]
            logger.info("正在打开投递页面: %s", job_name)

            await self.page.goto(job_link, wait_until="networkidle", timeout=20000)
            ...
            if any(kw in btn_text for kw in ["已申请", "已投递", "已投"]):
                result["message"] = "已投递过该岗位"
                result["success"] = True
                return result

            if any(kw in btn_text for kw in ["停止招聘", "已关闭"]):
                result["message"] = "岗位已关闭"
                return result

            if confirm:
                result["message"] = f"待确认投递: {job_name}"
            ...
            # 检查二次确认弹窗
            try:
                for sel in [".dialog button:has-text('确定')", ".dialog button:has-text('提交')",
            ...
            result["success"] = True
            result["message"] = "投递成功"
            logger.info("投递成功: %s", job_name)
            db.increment_request_count()
            db.insert_application(job["job_id"], "applied")

        except Exception as e:
            result["message"] = f"投递异常: {str(e)[:100]}"
            logger.error("投递失败: %s", e)

        return result

    async def batch_apply(self, jobs: list, min_score: float = 8.5,
                          max_apply: int = 5, confirm: bool = True) -> list:
        """批量自动投递高匹配岗位"""
        ...
        for job in jobs:
            if job.get("match_score", 0) < aconfig.get("min_score", min_score):
                continue
            if applied >= aconfig.get("max_daily", max_apply):
                break

            if applied > 0:
                await asyncio.sleep(aconfig.get("apply_interval", 5.0))

            result = await self.apply_to_job(
                job, confirm=aconfig.get("confirm_before_apply", confirm)
            )
            results.append(result)
            if result.get("success"):
                applied += 1
                db.update_job_status(job["job_id"], "applied")

        logger.info("批量投递: %d 次尝试, %d 成功", len(results), applied)
        return results
